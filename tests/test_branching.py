...
from turbulence.config.sut import Service
...
def scenario_runner(template_engine, sut_config):
    ...
    return ScenarioRunner(template_engine, sut_config, client_pool=pool)
...
async def test_conditional_skip(scenario_runner):
    ...
    results = []
    async for r in scenario_runner.execute_flow(scenario, context):
        results.append(r)
