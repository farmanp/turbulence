...
async def test_variation_end_to_end_flow(tmp_path):
    ...
    scenario_runner = ScenarioRunner(template_engine, sut_config, client_pool=pool)

    # We'll mock the _execute_action to see the flow but we need to verify RENDERING
    async def mock_execute_action(action, context):
    ...
    results = []
    async for step_idx, _action, _observation, _updated_ctx in scenario_runner.execute_flow(scenario, rendered_context):
