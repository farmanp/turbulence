...
**User Story:**
As a developer
I want to inject pressure into workflows
So that I can test my system's resilience to real-world conditions like latency and failures

**Success Looks Like:**
Configurable pressure injection that adds latency, forces timeouts, and triggers retry storms to stress test service resilience.

## 2. Context & Constraints (Required)
**Background:**
Production systems experience pressure—network latency, service timeouts, clients retrying failed requests. Turbulence must simulate these conditions to verify that systems handle them gracefully without data corruption or cascading failures.
...
Given pressure config with timeout_after_ms: 100
...
Given pressure config with retry_count: 3
When an HTTP action executes
...
Given pressure config targeting only "payments" service
When executing actions against "api" and "payments"
Then only "payments" actions experience pressure
And "api" actions execute normally

**Scenario: Per-action turbulence**
Given pressure config for action "process_payment" only
...
**Allowed to Change:**
- Create src/turbulence/pressure/__init__.py
- Create src/turbulence/pressure/engine.py
- Create src/turbulence/pressure/config.py
- Integrate pressure with action runners
...
- feat(pressure): add pressure engine with latency, timeout, and retry injection

## 6. Verification & Definition of Done (Required)
- [ ] Acceptance criteria pass
- [ ] Tests verify each pressure type
- [ ] Code reviewed
- [ ] No breaking changes
